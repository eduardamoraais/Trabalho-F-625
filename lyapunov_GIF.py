...
g = 9.8
mass_ratio = [2*10**i for i in linspace(-2, 2, 100)] #razões entre massas a serem consideradas
length_ratio = [2*10**i for i in linspace(-2, 2, 100)] #razões entre comprimentos dos fios a serem consideradas
m1, l1 = 1.0, 1.00 #massa e comprimento do fio do pêndulo interno
...
name = int(1)
for angle in tqdm([0.02*c*pi/2 for c in range(1, 31)], desc = 'Angle loop'):
  ...
  lmax = max(LYAPUNOV) #para se definir um valor vmax na escala do gráfico de densidade

  #plot do gráfico de densidade dos expoentes

  fig, ax = subplots()
  ax = imshow(LYAPUNOV, origin = 'lower', extent = [-2, 2, -2, 2], vmax = 0.05)
  xlabel('$\log_{10}(L_2/L_1)$')
  ylabel('$\log_{10}(M_2/M_1)$')
  title(f'$Angle = {round(angle/pi, 3):.2f} \pi$')
  cb = fig.colorbar(ax)
  cb.set_label('$\lambda$')
  savefig('lyapunovGIF' + str(name))
  name += int(1)